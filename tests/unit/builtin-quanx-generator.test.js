...
describe('Quantumult X 内置生成器', () => {
    it('should generate managed config and proxy sections', () => {
        const result = generateBuiltinQuanxConfig(
            'ss://YWVzLTEyOC1nY206cGFzc3dvcmQ=@1.2.3.4:443#HKNode',
            {
    ...
    it('should emit parser-compatible shadowsocks lines', () => {
        const result = generateBuiltinQuanxConfig(
            'ss://YWVzLTEyOC1nY206cGFzc3dvcmQ=@1.2.3.4:443#SSNode'
        );
    ...
    it('should skip hysteria2 when rendering QuanX because target rejects it', () => {
        const generated = generateBuiltinQuanxConfig(
            'hysteria2://97fe958d-2c3e-4994-9df0-293ccdb5f39@x-mg.xueshan168.cc:20201?sni=x-mg.xueshan168.cc&allowInsecure=1#Stable%20-%20%E8%B7%9D%E7%A6%BB%E4%B8%8B%E6%AC%A1%E9%87%8D%E7%BD%AE%E5%89%A9%E4%BD%99%EF%BC%9A%2019%20%E5%A4%A9'
        );
    ...
    it('should emit Quantumult X compatible vless reality syntax', () => {
        ...
        expect(generated).toContain(
            'vless=34.21.195.221:65043, password=52e98ca8-0671-451a-940a-961b8k, method=none, obfs=over-tls, obfs-host=addons.mozilla.org, reality-base64-pubkey=QpmZt9PSrjOltpKalxbwoCYTkOkRPwVnNInqTCRic, reality-hex-shortid=0123456789abcdef'
        );
    ...
    it('should avoid obfs-uri when xhttp is mapped to over-tls for QuanX', () => {
        ...
        expect(generated).toContain(
            'vless=cf.tencentapp.cn:443, password=67d08f0c-d864-4c58-8491-ffa03097c60b, method=none, obfs=over-tls, obfs-host=uk.xxxxxxxxxxxxx.xxx.kg'
        );
    ...
    it('should render Shadowsocks v2ray-plugin TLS websocket as wss without over-tls', () => {
        const generated = generateBuiltinQuanxConfig(
            'ss://YWVzLTEyOC1nY206OGI1NDBmNWMtNjJjMS00NDkyLTgzZjYtOTQ0ZjUzNGFkMDI2@79.137.195.150:444?plugin=v2ray-plugin%3Bmode%3Dwebsocket%3Bhost%3Dfofo.sinemp.nyc.mn%3Bpath%3D%2Fproxyip%3Dproxyip.cmliussss.net%3Fenc%3Daes-128-gcm%3Bmux%3Dfalse%3Btls#DE'
        );
        const line = generated.split('\n').find((item) => item.startsWith('shadowsocks='));

        expect(line).toBe(
            'shadowsocks=79.137.195.150:444, method=aes-128-gcm, password=8b540f5c-62c1-4492-83f6-944f534ad026, obfs=wss, obfs-uri=/proxyip=proxyip.cmliussss.net?enc=aes-128-gcm, obfs-host=fofo.sinemp.nyc.mn, tag=🇩🇪 DE'
        );
    ...
    it('should document Quantumult X 1.6.0 AnyTLS support in builtin output and parser round-trip', () => {
        const generated = generateBuiltinQuanxConfig(
            'anytls://pass-anytls@anytls.example.com:443/?sni=anytls.example.com&alpn=h2,h3&allowInsecure=1#AnyTLS-v1.6.0'
        );
