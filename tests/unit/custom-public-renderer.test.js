...
describe('CustomPublicRenderer iframe mode', () => {
    ...
    it('renders sanitized srcdoc iframe for iframe-srcdoc mode with sandbox enabled', () => {
        const wrapper = mountRenderer({
            props: {
                content:
                    '<!DOCTYPE html><html><head><title>Demo</title></head><body><div onclick="alert(1)">Hello</div><script>alert(1)</script></body></html>',
                css: '',
                config: {
                    customPage: {
                        type: 'iframe-srcdoc',
    ...
    it('sanitizes inline custom HTML before v-html rendering', () => {
        const wrapper = mountRenderer({
            props: {
                content:
                    '<div onclick="alert(1)">Hello <img src=x onerror="alert(2)"></div><script>alert(3)</script>{{ profiles }}',
                css: '',
