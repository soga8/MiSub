...
import {
    handleApiRequest,
    handleExternalFetchRequest,
    handleSubconverterTestRequest,
} from '../../functions/modules/api-router.js';
import {
...
import {
    ...
    renewAuthSession,
} from '../../functions/modules/auth-middleware.js';
...
describe('security hardening', () => {
    ...
    it('rejects oversized public preview JSON bodies before outbound fetch', async () => {
        const fetchSpy = vi
            ...
            .mockResolvedValue(new Response('should-not-fetch'));
        const response = await handlePreviewContentRequest(
            new Request('https://example.com/api/preview/content', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    it('keeps browser login sessions valid for thirty days', async () => {
        ...
        const diagnostic = await getAuthSessionDiagnostic(
            {
                headers: {
                    get: (name) => (name.toLowerCase() === 'cookie' ? `auth_session=${token}` : ''),
                },
            },
            env
        );
    ...
    it('marks sessions for renewal after seven days without renewing earlier sessions', async () => {
        const env = { MISUB_KV: createKv(), COOKIE_SECRET: 'stable-cookie-secret' };
        const recentToken = await createSignedToken(
            env.COOKIE_SECRET,
            String(Date.now() - SESSION_RENEW_THRESHOLD + 1000)
        );
        const matureToken = await createSignedToken(
            env.COOKIE_SECRET,
            String(Date.now() - SESSION_RENEW_THRESHOLD - 1000)
        );
        ...
        const recent = await getAuthSessionDiagnostic(
            {
                headers: {
                    get: (name) =>
                        name.toLowerCase() === 'cookie' ? `auth_session=${recentToken}` : '',
                },
            },
            env
        );
        const mature = await getAuthSessionDiagnostic(
            {
                headers: {
                    get: (name) =>
                        name.toLowerCase() === 'cookie' ? `auth_session=${matureToken}` : '',
                },
            },
            env
        );
    ...
    it('does not renew sessions before the threshold or after expiry', async () => {
        ...
        const expiredToken = await createSignedToken(
            env.COOKIE_SECRET,
            String(Date.now() - SESSION_DURATION - 1000)
        );
        ...
        const expiredResponse = await renewAuthSession(
            {
                ...
                headers: {
                    get: (name) =>
                        name.toLowerCase() === 'cookie' ? `auth_session=${expiredToken}` : '',
                },
            },
            env,
            new Response('ok')
    ...
    it('does not expose public auth debug endpoints in production', async () => {
        ...
        const debugResponse = await handleApiRequest(
            new Request('https://example.com/api/auth_debug'),
            env
        );
        const checkResponse = await handleApiRequest(
            new Request('https://example.com/api/auth_check', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    it('supports cron query secret compatibility and Authorization bearer', async () => {
        ...
        try {
            ...
            expect(warnSpy).toHaveBeenCalledWith(
                '[Storage] No KV binding found, using noop adapter'
            );
            expect(infoSpy).toHaveBeenCalledWith(
                '[Cron] Starting parallel update for 0 subscriptions'
            );
        } finally {
            warnSpy.mockRestore();
            infoSpy.mockRestore();
    ...
    it('reports backend test failure when converter output is not usable', async () => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValue(
            new Response('No nodes were found!', { status: 200 })
        );
    ...
    it('does not pass insecureSkipVerify when previewing external content', async () => {
        const fetchSpy = vi
            .spyOn(globalThis, 'fetch')
            .mockResolvedValue(new Response('dm1lc3M6Ly9leGFtcGxl', { status: 200 }));
    ...
    it('redacts access-log token and profile identifiers before payload storage and dedupe fingerprinting', async () => {
        ...
        try {
            logAccessSuccess({
                ...
                env: {},
                request: new Request(
                    'https://example.com/profile-share-token/private-profile-slug-secret',
                    {
    ...
    it('blocks private and loopback addresses in external fetch and preview APIs', async () => {
        ...
        const externalResponse = await handleExternalFetchRequest(
            new Request('https://example.com/api/fetch_external_url', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        const previewResponse = await handlePreviewContentRequest(
            new Request('https://example.com/api/preview/content', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    it('allows only explicit trusted external hostnames for public fetch APIs', async () => {
        ...
        const rejected = await handlePreviewContentRequest(
            new Request('https://example.com/api/preview/content', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        const accepted = await handlePreviewContentRequest(
            new Request('https://example.com/api/preview/content', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    it('redacts sensitive fields from exported settings and subscriptions', async () => {
                ...
                misub_subscriptions_v1: JSON.stringify([
                    {
                        id: 'sub-1',
                        name: 'Airport',
                        url: 'https://airport.example/link/export-token-secret?token=secret-query',
                    },
                ]),
                misub_profiles_v1: JSON.stringify([]),
            }),
        ...
        const response = await handleExportDataRequest(
    ...
    it('redacts debug subscription result URLs and full raw result', async () => {
        ...
        const fetchSpy = vi
            .spyOn(globalThis, 'fetch')
            .mockResolvedValue(new Response(encoded, { status: 200 }));

        const response = await handleDebugSubscriptionRequest(
            new Request('https://example.com/api/debug_subscription', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
