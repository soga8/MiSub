...
describe('handleTelegramWebhook', () => {
    ...
    it('rejects webhook requests when secret is missing', async () => {
        ...
        const { handleTelegramWebhook } =
            await import('../../functions/modules/handlers/telegram-webhook-handler.js');
        try {
    ...
    it('stores bindings per telegram user and auto-binds imports to the correct profile', async () => {
        ...
        try {
            ...
            expect(infoSpy).toHaveBeenCalledWith(
                '[Telegram Push] Cleared 1 node caches after node import'
            );
            expect(infoSpy).toHaveBeenCalledWith(
                '[Telegram Push] User 1 added 1 items (Ignored 0)'
            );
        } finally {
            infoSpy.mockRestore();
        }
            ...
            subscriptions: [
                {
                    id: 'node-1',
                    name: 'HK VLESS',
                    url: 'vless://uuid@example.com:443#HK',
                    enabled: true,
                },
                {
                    id: 'airport-1',
    ...
    it('opens an airport subscription from an old mixed /list callback index instead of reporting object missing', async () => {
        ...
        expect(fetchBodies).not.toEqual(
            expect.arrayContaining([
