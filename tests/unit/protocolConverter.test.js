        ...
        describe('Hysteria2', () => {
            ...
            it('应保留Hysteria2 realm配置参数', () => {
                ...
                expect(result).toContain(
                    'stun-servers=stun.sip.us%3A3478%2Cstun.nextcloud.com%3A3478'
                );
            });
        });
...
`;
    ...
    describe('parseClientConfig', () => {
        it('应自动识别 Surge / Loon / QX 配置', () => {
            const surge = parseClientConfig(
                '[Proxy]\nss=Test SS,1.2.3.4,8388,aes-256-gcm,password123'
            );
            const loon = parseClientConfig(
                '[Proxy]\nTest SS = Shadowsocks, 1.2.3.4, 8388, aes-128-gcm, password123'
            );
            const quanx = parseClientConfig(
                '[server_local]\nshadowsocks=Test SS, 1.2.3.4, 443, aes-128-gcm, password'
            );
            const surgeWireguard = parseClientConfig(
