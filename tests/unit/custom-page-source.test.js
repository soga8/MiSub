...
describe('parseCustomPageSource', () => {
    it('extracts body and style from full html document', () => {
        const parsed = parseCustomPageSource(
            `<!DOCTYPE html><html><head><title>Demo</title><meta name="description" content="demo description"><style>body{background:#000}.box{color:#fff}</style></head><body id="app-root"><div class="box">hello</div><script src="https://cdn.example.com/jquery.js"></script><script>console.log(1)</script></body></html>`
        );
    ...
    it('keeps fragment html usable while extracting style tags', () => {
        const parsed = parseCustomPageSource(
            '<style>.a{color:red}</style><section class="a">x</section>'
        );
