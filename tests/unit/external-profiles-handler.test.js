...
describe('external profiles and preview handlers', () => {
        ...
        const env = createEnv({
            subscriptions: [
                { id: 'sub-1', name: 'Airport A', url: 'https://example.com/sub-a', enabled: true },
                {
                    id: 'node-1',
                    name: 'HK 01',
                    url: 'vless://uuid@example.com:443#HK01',
                    enabled: true,
                },
            ],
        });
    ...
    it('adds and removes profile resource references through dedicated endpoints', async () => {
        ...
        const addSubs = await handleApiRequest(
            createRequest('/profiles/profile-1/subscriptions', {
                method: 'POST',
        ...
        const addNodes = await handleApiRequest(
            createRequest('/profiles/profile-1/manual-nodes', {
                method: 'POST',
        ...
        const removeSubs = await handleApiRequest(
            createRequest('/profiles/profile-1/subscriptions', {
                method: 'DELETE',
        ...
        const removeNodes = await handleApiRequest(
            createRequest('/profiles/profile-1/manual-nodes', {
                method: 'DELETE',
    ...
    it('returns lightweight profile previews with counts, protocol stats, and typed sources', async () => {
        ...
        const response = await handleApiRequest(
            createRequest('/profiles/profile-1/preview', {
                method: 'POST',
                body: {},
            }),
            env
