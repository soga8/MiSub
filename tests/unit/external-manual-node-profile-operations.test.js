...
describe('external manual-node/profile validate and refresh operations', () => {
    ...
    it('validates a candidate manual node url and returns parsed node details', async () => {
        const response = await handleApiRequest(
            createRequest('/manual-nodes/validate', {
                method: 'POST',
                body: {
                    name: 'SG 01',
    ...
    it('refreshes a profile by updating referenced remote subscriptions and returning aggregate counts', async () => {
                        ...
                        `proxies:\n  - name: HK 01\n    type: trojan\n    server: hk.example.com\n    port: 443\n    password: pass\n`,
                        {
                            ...
                            headers: {
                                'subscription-userinfo':
                                    'upload=100; download=200; total=300; expire=400',
                            },
                        }
                    );
                }
        ...
        const response = await handleApiRequest(
            createRequest('/profiles/profile-1/refresh', {
                method: 'POST',
                body: {},
            }),
            env
        ...
        expect(subscriptionsBody.data).toEqual([
            expect.objectContaining({
                id: 'sub-1',
                nodeCount: 1,
                userInfo: { upload: 100, download: 200, total: 300, expire: 400 },
            }),
            expect.objectContaining({
                id: 'sub-2',
                nodeCount: 0,
