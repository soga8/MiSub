...
export async function fetchWithRetry(url, init = {}, options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(
                `[Fetch Retry] Attempt ${attempt + 1}/${maxRetries} failed for ${url}: ${error.message}. Retrying in ${delay / 1000}s...`
            );
...
export async function collectWithinTimeout(promises, timeoutMs) {
    ...
    // 等待全部完成或超时信号
    const raceResult = await Promise.race([Promise.all(wrappedPromises), timeoutPromise]);
    ...
    if (raceResult === 'TIMEOUT') {
        ...
        console.warn(
            `[Subscription Fetch] Timeout of ${timeoutMs}ms reached. Handling partial results...`
        );
        ...
        // 全都完成了
        results = raceResult.filter((r) => r.status === 'fulfilled' && r.value).map((r) => r.value);
    }
