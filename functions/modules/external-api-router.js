...
export async function handleExternalApiRequest(request, env) {
    ...
    if (subscriptionMatch) {
        return await handleExternalSubscriptionsRequest(
            request,
            env,
    ...
    if (manualNodeMatch) {
        return await handleExternalManualNodesRequest(
            request,
            env,
            decodeURIComponent(manualNodeMatch[1])
        );
    }
    ...
    if (previewMatch) {
        ...
        return await handleExternalPreviewRequest(
            request,
            env,
    ...
    if (profileMatch) {
        return await handleExternalProfilesRequest(request, env, {
            id: decodeURIComponent(profileMatch[1]),
        });
    }
