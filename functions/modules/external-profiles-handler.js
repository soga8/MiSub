...
async function validateProfileReferences(
    ...
    if (subscriptionIds !== undefined) {
        for (const id of subscriptionIds) {
            ...
            if (!isRemoteSubscription(item))
                return createExternalError(
                    'invalid_profile_reference',
                    `Subscription ${id} is not a remote subscription`,
                    400
                );
        }
    }
        ...
        for (const id of manualNodeIds) {
            ...
            if (!item)
                return createExternalError(
                    ...
                    `Manual node ${id} not found`,
                    400
                );
            if (!isManualNode(item))
                return createExternalError(
                    'invalid_profile_reference',
                    `Manual node ${id} is not a manual node`,
                    400
                );
        }
    }
        ...
        summary: {
            ...
            totalNodes:
                results
                    ...
                    .reduce((sum, item) => sum + (Number(item.nodeCount) || 0), 0) +
                manualNodes.length,
        },
    };
}

export async function handleExternalProfilesRequest(
    request,
    env,
    { id = null, relation = null, action = null } = {}
) {
    ...
    if (request.method === 'GET' && !id) {
        ...
        const filtered = profiles.filter((profile) => {
            if (enabledFilter !== undefined && (profile.enabled !== false) !== enabledFilter)
                return false;
            if (keyword) {
                const haystack =
    ...
    if (request.method === 'POST' && id && action === 'refresh') {
        ...
        if (!profile) return createExternalError('profile_not_found', 'Profile not found', 404);
        return createExternalSuccess(
            await refreshProfileSubscriptions(storageAdapter, env, profile)
        );
    }
    ...
    if (request.method === 'POST' && !id) {
        ...
        if (!String(payload?.name || '').trim())
            return createExternalError('profile_name_required', 'Profile name is required', 400);
        const subscriptionIds =
            payload.subscriptionIds !== undefined ? dedupe(payload.subscriptionIds) : [];
        const manualNodeIds =
            payload.manualNodeIds !== undefined ? dedupe(payload.manualNodeIds) : [];
        const validationError = await validateProfileReferences(
            storageAdapter,
            subscriptionIds,
            manualNodeIds
        );
    ...
    if (request.method === 'PATCH' && id && !relation) {
        ...
        const payload = await readExternalJson(request);
        const subscriptionIds =
            payload.subscriptionIds !== undefined ? dedupe(payload.subscriptionIds) : undefined;
        const manualNodeIds =
            payload.manualNodeIds !== undefined ? dedupe(payload.manualNodeIds) : undefined;
        const validationError = await validateProfileReferences(
            storageAdapter,
            subscriptionIds,
            manualNodeIds
        ...
        const updated = {
            ...
            ...(payload.customId !== undefined
                ? { customId: String(payload.customId || '').trim() }
                : {}),
            ...(payload.target !== undefined
                ? { target: String(payload.target || 'clash').trim() || 'clash' }
                : {}),
            ...(payload.sortIndex !== undefined
                ? { sortIndex: Number(payload.sortIndex) || 0 }
                : {}),
    ...
    if (
        ...
        id &&
        (relation === 'subscriptions' || relation === 'manual-nodes')
    ) {
        const current = await getProfileById(storageAdapter, id);
        if (!current) return createExternalError('profile_not_found', 'Profile not found', 404);
        const payload = await readExternalJson(request);
