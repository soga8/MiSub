...
async function removeManualNodeIdFromProfiles(storageAdapter, id) {
    ...
    for (const profile of profiles) {
        ...
        const updated = {
            ...profile,
            manualNodes: manualNodes.filter((item) => item !== id),
            updatedAt: nowIso(),
        };
...
export async function handleExternalManualNodesRequest(request, env, selector = null) {
    ...
    if (action === 'validate') {
        ...
        if (!candidateUrl)
            return createExternalError(
                ...
                'Manual node URL is required',
                400
            );
        if (!isManualNode({ url: candidateUrl }))
            return createExternalError(
                'invalid_manual_node_url',
                'Manual node URL must use a supported node protocol',
                400
            );
    ...
    if (request.method === 'GET' && id) {
        ...
        if (!item)
            return createExternalError('manual_node_not_found', 'Manual node not found', 404);
