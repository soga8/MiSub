<script setup>
    ...
    const migrateFromLegacy = () => {
        ...
        if (regex?.enabled && regex.rules?.length > 0) {
            ops.push({
                ...
                type: 'rename',
                enabled: true,
                params: { regex: JSON.parse(JSON.stringify(regex)) },
            });
        }
        ...
        if (dedup?.enabled) {
            ops.push({
                ...
                type: 'dedup',
                enabled: true,
                params: JSON.parse(JSON.stringify(dedup)),
            });
        }
        ...
        if (sort?.enabled && sort.keys?.length > 0) {
            ops.push({
                ...
                type: 'sort',
                enabled: true,
                params: JSON.parse(JSON.stringify(sort)),
            });
        }
        ...
        switch (type) {
            ...
            case 'rename':
                return {
                    ...
                    template: { enabled: false, template: '' },
                };
            case 'script':
                return { code: '', url: '' };
            case 'sort':
            ...
            case 'dedup':
                return { mode: 'serverPort', includeProtocol: true, prefer: { protocolOrder: [] } };
            default:
                return {};
        }
    };
    ...
    const getOperatorIcon = (type) => {
        switch (type) {
            ...
            case 'script':
                return '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>';
            case 'sort':
                return '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" /></svg>';
            case 'dedup':
                return '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>';
        }
    };
...
<template>
    <div class="space-y-4">
        ...
        <div
            v-if="canMigrate"
            class="group relative mb-6 overflow-hidden rounded-xl border border-amber-200/50 bg-gradient-to-br from-amber-50 to-orange-50 p-5 dark:border-amber-500/20 dark:from-amber-900/10 dark:to-orange-900/10"
        >
            <div class="relative z-10 flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div
                        class="w-10 h-10 rounded-2xl bg-amber-500 text-white flex items-center justify-center shadow-lg shadow-amber-500/20"
                    >
                        <svg
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-amber-900 dark:text-amber-200">
                            {{ t('operators.legacyConfigFound') }}
                        </h4>
                        <p class="mt-0.5 text-[10px] text-amber-700/70 dark:text-amber-400/60">
                            {{ t('operators.legacyConfigDesc') }}
                        </p>
                    </div>
                </div>
                <button
                    @click="migrateFromLegacy"
                    class="whitespace-nowrap rounded-xl bg-amber-500 px-5 py-2 text-[11px] font-bold text-white shadow-md shadow-amber-500/20 transition-all hover:bg-amber-600 active:scale-95"
                >
        ...
        <div
            v-if="modelValue.length === 0 && !canMigrate"
        ...
        <div v-else class="space-y-2">
            <div
                v-for="(op, index) in modelValue"
                :key="op.id || index"
                :class="[
                    'group overflow-hidden rounded-2xl border bg-white transition-all dark:bg-gray-800',
                    expandedIndex === index
                        ? 'border-indigo-500/50 ring-4 ring-indigo-500/5 shadow-xl'
                    ...
                    <div class="flex items-start justify-between gap-3 sm:items-center">
                        <div class="flex items-center gap-3 min-w-0">
                            ...
                            <div>
                                ...
                                <p
                                    class="mt-1 text-[11px] text-gray-500 dark:text-gray-400 sm:hidden"
                                >
                                    {{
                                        op.enabled
                                            ? t('operators.enabled')
                        ...
                        <div class="flex items-center gap-2">
                            <div
                                class="flex items-center gap-1.5 pr-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                            >
                                <button
                                ...
                                <button
                                    @click="(e) => toggleOperator(e, index)"
                ...
                >
                    ...
                    >
                        <div class="p-4 sm:p-5">
                            <FilterEditor
                                v-if="op.type === 'filter'"
                                :modelValue="op.params"
                                @update:modelValue="(val) => updateOperatorParams(index, val)"
                            />
                            <RenameEditor
                                v-else-if="op.type === 'rename'"
                                :modelValue="op.params"
                                @update:modelValue="(val) => updateOperatorParams(index, val)"
                            />
                            <SortEditor
                                v-else-if="op.type === 'sort'"
                                :modelValue="op.params"
                                @update:modelValue="(val) => updateOperatorParams(index, val)"
                            />
                            <DedupEditor
                                v-else-if="op.type === 'dedup'"
                                :modelValue="op.params"
                                @update:modelValue="(val) => updateOperatorParams(index, val)"
                            />
                            ...
                            <div v-else-if="op.type === 'script'" class="space-y-4">
                                <Input
                                    ...
                                    @update:modelValue="
                                        (val) =>
                                            updateOperatorParams(index, { ...op.params, url: val })
                                    "
        ...
        <div class="pt-6">
            <div
                class="grid grid-cols-2 gap-2 rounded-2xl border border-gray-200 bg-gray-100/50 p-1 dark:border-gray-800 dark:bg-gray-900/50 sm:inline-flex sm:flex-wrap"
            >
                <button
