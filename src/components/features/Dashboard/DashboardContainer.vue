/** * Dashboard 主容器组件 * @author MiSub Team */
...
<script setup>
    ...
    const initializeState = () => {
        ...
        if (props.data) {
            ...
            initialSubs.value = subsData.filter(
                (item) => item.url && /^https?:\/\//.test(item.url)
            );
            initialNodes.value = subsData.filter(
                (item) => !item.url || !/^https?:\/\//.test(item.url)
            );
    ...
    const handleBulkImport = (importedSubscriptions) => {
        ...
        showToast(
            t('subscriptions.importedCount', { count: importedSubscriptions.length }),
            'success'
        );
    };
...
<template>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div
                v-else
                class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 items-start"
            >
