...
const TEMPLATE_VARIABLE_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: '<%all_strategy_groups%>', example: '所有策略组名称合集' }
    ]
  }
];

const assets = computed(() => TRANSFORM_ASSETS.configs.filter(item => {
...
const helperText = computed(() => {
  if (props.excludeBuiltinAssets) {
    return '第三方后端仅支持远程模板 URL，不支持 MiSub 内置预设。';
  }
  return '适用于统一模板渲染。';
});
</script>
  ...
  <div>
    <div v-if="!isCustom" class="relative">
      ...
      >
        ...
        <option value="custom" class="font-bold text-indigo-600 dark:text-indigo-400 border-t">自定义输入...</option>
      </select>
