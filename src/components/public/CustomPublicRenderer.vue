<script setup>
    ...
    const removeExternalStylesheets = () => {
        document
            .querySelectorAll(`[${stylesheetDataAttr}="true"]`)
    ...
    const isIframeMode = computed(() =>
        ['iframe-srcdoc', 'iframe-url'].includes(customPageType.value)
    );
    ...
    const iframeSrcdoc = computed(() =>
        customPageType.value === 'iframe-srcdoc' ? sanitizeHtml(props.content || '') : undefined
    );
    ...
    const iframeSrc = computed(() =>
        customPageType.value === 'iframe-url'
            ? safeIframeUrl(customPageConfig.value.iframeUrl)
            : undefined
    );
    const iframeAllowFullscreen = computed(() =>
        customPageConfig.value.iframeAllowFullscreen === true ? true : undefined
    );
    ...
    const renderedHtml = computed(() => {
        ...
        // 1. 文本占位符替换
        const nodeCount = props.profiles.reduce(
            (sum, p) => sum + (p.subscriptionCount || 0) + (p.manualNodeCount || 0),
            0
        );
        const textVars = {
            version: '2.7.0',
    ...
    <div class="custom-public-renderer">
        ...
        <template v-else>
            ...
            <template v-if="isReady">
                <Teleport
                    v-if="hasSlot('profiles')"
                    to='[data-slot="profiles"]'
                ...
                <Teleport
                    v-if="hasSlot('announcements')"
                    to='[data-slot="announcements"]'
                    :disabled="disableTeleport"
                >
                ...
                <Teleport
                    v-if="hasSlot('guestbook')"
                    to='[data-slot="guestbook"]'
                    :disabled="disableTeleport"
                >
                ...
                <Teleport
                    v-if="hasSlot('theme_toggle')"
                    to='[data-slot="theme_toggle"]'
                    :disabled="disableTeleport"
                >
                    <button
