<script setup>
    ...
    const platformOptions = [
        ...
        },
        {
            value: 'macos',
            label: 'macOS',
            class: 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200',
        },
        {
            value: 'linux',
            label: 'linux',
            class: 'bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-200',
        },
        {
            value: 'android',
            label: 'android',
        ...
        },
        {
            value: 'ios',
            label: 'iOS',
            class: 'bg-gray-800 text-white dark:bg-white dark:text-gray-900',
        },
        {
            value: 'HarmonyOS',
            label: 'harmonyOS',
    ...
    const saveClientOrder = async () => {
        ...
        try {
            ...
            showToast(
                t('settings.clientOrderSaveFailedWithMessage', { message: e.message }),
                'error'
            );
        } finally {
    ...
    <div class="space-y-6">
        ...
        >
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <SectionHeader
                    :title="t('settings.clientManagerTitle')"
                    :description="t('settings.clientManagerDesc')"
                    tone="blue"
                >
                    <template #icon>
                        <svg
                        ...
                        >
                            <path
                                ...
                                stroke-width="2"
                                d="M9.75 17L9 20l-1-3m1.75 0h3.5M12 3v14m0 0l3-3m-3 3l-3-3"
                            />
                        </svg>
                    </template>
                </SectionHeader>
                <div class="flex gap-2 w-full md:w-auto">
                    <button
                        @click="handleInit"
                        class="flex-1 md:flex-none px-3 py-1.5 text-center text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 misub-radius-lg border border-gray-300 dark:border-gray-600"
                    >
                        {{ t('settings.clientResetDefault') }}
                    </button>
                    <button
                        @click="handleAdd"
                        class="flex-1 md:flex-none px-4 py-2 text-center text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 misub-radius-lg shadow-sm shadow-primary-500/20"
                    >
                        {{ t('settings.clientAdd') }}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
                <div class="flex items-start gap-3 min-w-0">
                    ...
                    >
                        <img
                            v-if="
                                client.icon &&
                                (client.icon.includes('/') || client.icon.startsWith('data:'))
                            "
                            :src="client.icon"
                            :alt="client.name"
                    ...
                    <div class="min-w-0 flex-1">
                        <h4 class="font-bold text-gray-900 dark:text-white truncate pr-1">
                            {{ client.name }}
                        </h4>
                        <p class="text-xs text-gray-500 truncate">{{ client.description }}</p>
                        <div class="flex flex-wrap gap-1 mt-2">
                            <span
                                v-for="p in client.platforms"
                                :key="p"
                ...
                <div
                    class="mt-4 grid grid-cols-2 sm:flex sm:flex-wrap gap-2 border-t border-gray-100 dark:border-gray-700 pt-3"
                >
                    <button
                        @click.stop="moveClient(index, -1)"
        ...
        <Modal
            ...
            :title="isNew ? t('settings.clientAddModalTitle') : t('settings.clientEditModalTitle')"
            size="5xl"
        >
            <template #body>
                <div class="space-y-6">
                            ...
                            >
                                <div class="text-center space-y-3 w-full">
                                    <label
                                        ...
                                        >{{ t('settings.clientIconPreview') }}</label
                                    >
                                    <div
                                    ...
                                    >
                                        <img
                                            v-if="
                                                editingClient.icon &&
                                                (editingClient.icon.includes('/') ||
                                                    editingClient.icon.startsWith('data:'))
                                            "
                                            :src="editingClient.icon"
                                ...
                                <div class="w-full space-y-4 mt-2">
                                    <div class="space-y-2">
                                        <label
                                            class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                                            >{{ t('settings.clientLocalIcon') }}</label
                                        >
                                        <input
                                            ref="iconFileInput"
                                            type="file"
                                        ...
                                        <div class="flex gap-2 justify-center">
                                            ...
                                            >
                                                ...
                                                <span>{{
                                                    uploadingIcon
                                                        ? t('settings.clientUploading')
                                                        : t('settings.clientUploadImage')
                                                }}</span>
                                            </button>
                                            <button
                                                v-if="editingClient.icon"
                                                @click="clearIcon"
                                                type="button"
                                        ...
                                        >
                                            {{ t('settings.clientUsingLocalIcon') }}
                                        </p>
                                    </div>
                                    <div class="space-y-2">
                                        <label
                        ...
                        <div class="md:col-span-7 lg:col-span-8 flex flex-col gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                                        >{{ t('settings.clientDescription') }}</label
                                    >
                                    <textarea
                            ...
                            <div>
                                <label
                                    ...
                                    >{{ t('settings.clientPlatforms') }}</label
                                >
                                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                                    <label
                                        ...
                                        :class="
                                            editingClient.platforms.includes(opt.value)
                                    ...
                                    >
                                        <input
                                            type="checkbox"
                                            :value="opt.value"
                                            v-model="editingClient.platforms"
                                            class="hidden"
                                        />
                                        <span class="text-sm font-medium">{{ opt.label }}</span>
                                        <div
                                            v-if="editingClient.platforms.includes(opt.value)"
                                            class="absolute top-2 right-2 w-2 h-2 bg-indigo-500 rounded-full"
                                        ></div>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                                <div>
                                    <label
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                                        >{{ t('settings.clientDownloadUrl') }}</label
                                    >
            ...
            <template #footer>
                <div class="flex justify-end gap-3">
                    <button
                        ...
                        class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 misub-radius-md hover:bg-gray-50 dark:hover:bg-gray-700"
                    >
                        {{ t('actions.cancel') }}
                    </button>
                    <button
        ...
        <Modal
            ...
            :title="t('settings.clientResetConfirmTitle')"
            :confirmText="t('settings.clientResetConfirmAction')"
        ...
        <Modal
            ...
            :title="t('settings.clientDeleteConfirmTitle')"
            :confirmText="t('settings.clientDeleteConfirmAction')"
            :cancelText="t('actions.cancel')"
            size="sm"
        >
            <template #body>
