export function parseCustomPageSource(content, css = '') {
    ...
    const styleBlocks = Array.from(doc.querySelectorAll('style')).map(
    ...
    doc.querySelectorAll('style, link[rel~="stylesheet"], meta, title').forEach((node) =>
        node.remove()
    );

    const body = doc.body;
    const extractedCss = normalizeDocumentCss(
        [...linkImports, ...styleBlocks, rawCss].filter(Boolean).join('\n\n')
    );
    const needsWrapper =
        hadFullDocument ||
        body.attributes.length > 0 ||
        /(^|[^\w-])(html|body|:root)(?=[^\w-]|$)/i.test(extractedCss);
    const html = needsWrapper ? wrapBodyContent(body) : body.innerHTML;
